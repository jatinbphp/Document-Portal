...
class CustomersModel extends Model
{
	...
	protected $allowedFields = [  'email', 'pwd', 'firstName','dateadd','billadd','shipadd','lsorder','totalsp','avgorder', 'isActive', 'lastName','filedata', 'profilePic',  'lastLogin',  'dateAdded'];
    ...
    public function getActionLink($path,$id,$view=null,$userdelete=null){

        $actionLinkVar = '';  

        $actionLinkVar .= '<a title="Edit Customer" href="' . base_url( 'customers/edit/'.$id). '" class="btn btn-warning" style="margin: 0px 5px 5px 0px;padding: 4px 9px;font-size: 14px;"><i class="fa fa-edit"></i></a>';

        if($userdelete!=1){
            $actionLinkVar .= '<a href="' . base_url( 'customers/delete/'.$id). '" title="Delete Customer" class="btn btn-danger deleteData deleteCustomers" style="margin: 0px 5px 5px 0px;padding: 4px 9px;font-size: 14px;" data-id="' . $id . '"><i class="fa fa-trash"></i></a>';
        }
    ...
    public function getCustomersRecord(){
