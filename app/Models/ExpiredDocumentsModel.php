...
class ExpiredDocumentsModel extends Model
{
	...
	protected $allowedFields = ['document_name', 'usertype_id', 'category_id', 'subcategory_id','document_files', 'comments','company_id'];
    ...
    {   
        ...
        if(!empty($whereNotData)){            
            foreach ($whereNotData as $fieldName => $fieldValue) {
                $this->where($fieldName.' <', $fieldValue); //this statement has been modified from checking not equal condition "!=" to less than equal condition "<"           
            } 
        }
