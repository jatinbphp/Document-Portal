...
class UserDocumentsModel extends Model
{
    ...
    public function getActionLink($path,$id,$type,$view=null,$userdelete=null){
        ...
        $actionLinkVar .= '<a title="Edit Documents" href="' . base_url( 'userDocuments/edit/'.$id). '" class="btn btn-warning" style="margin: 0px 5px 5px 0px;padding: 4px 9px;font-size: 14px;"><i class="fa fa-edit"></i></a>';

        if($userdelete!=1){
            $actionLinkVar .= '<a href="' . base_url( 'userDocuments/delete/'.$id). '" title="Delete Document" class="btn btn-danger deleteData deleteDocuments" style="margin: 0px 5px 5px 0px;padding: 4px 9px;font-size: 14px;" data-id="' . $id . '"><i class="fa fa-trash"></i></a>';
        }
